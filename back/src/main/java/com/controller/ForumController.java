    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            BeanUtils.copyProperties( forum , view );//把实体数据重构到view中
                ...
                //级联表
                ZiyuanzheEntity ziyuanzhe = ziyuanzheService.selectById(forum.getZiyuanzheId());
                if(ziyuanzhe != null){
                    BeanUtils.copyProperties( ziyuanzhe , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setZiyuanzheId(ziyuanzhe.getId());
                }
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setZiyuanzheId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("ziyuanzhe_id", forum.getZiyuanzheId())
...
//                            forumEntity.setZiyuanzheId(Integer.valueOf(data.get(0)));   //自愿者 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    ZiyuanzheEntity ziyuanzhe = ziyuanzheService.selectById(forum.getZiyuanzheId());
                if(ziyuanzhe != null){
                    BeanUtils.copyProperties( ziyuanzhe , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setZiyuanzheId(ziyuanzhe.getId());
                }
                    UsersEntity users = usersService.selectById(forum.getUsersId());
                if(users != null){
